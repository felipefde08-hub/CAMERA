...
function startOperationsStream() {
  ...
  try {
    ...
    operationsStream.addEventListener("open", () => {
      notify("Tempo real conectado", "Painel recebendo atualizações operacionais.", "success", 2500);
    });
    operationsStream.addEventListener("error", () => {
      notify("Tempo real instável", "O painel continua funcionando por atualização manual.", "warning", 5000);
...
async function loadProductivity() {
  try {
    ...
    document.querySelector("#productivity-kpis").innerHTML = [
      metricItem("Score operacional", `${payload.score}%`, "Baseado em sinais objetivos"),
    ...
    document.querySelector("#productivity-cameras").innerHTML = payload.cameras.length
      ...
      : emptyState("Sem câmeras", "Cadastre e ligue Vision nas câmeras para medir produtividade.");
    ...
    document.querySelector("#productivity-machines").innerHTML = machines.length
      ...
      : mappedMachines.length
        ? mappedMachines.map((machine) => objectLine(
            ...
            `${machine.requires_operator ? "requer operador" : "operador opcional"} · ativo mapeado`
          )).join("")
        : emptyState("Nenhuma máquina mapeada", "Mapeie máquinas para contextualizar produtividade e segurança.");
    document.querySelector("#productivity-limitations").innerHTML = [
      objectLine("Leitura assistiva", "O sistema aponta sinais; a conclusão final deve ser revisada por operador.", "Produtividade não é julgamento automático."),
  ...
  return `
    <article class="ops-row">
      <div>
        ...
        <span>Score ${camera.score}% · ${counts.people || 0} pessoas · ${counts.machines || 0} detectadas · ${(camera.mapped_machines || []).length} mapeadas · ${counts.phones || 0} celulares</span>
        <small>${signals.length ? signals.map((signal) => signal.label).join(" · ") : "Sem sinais de produtividade no momento"}</small>
      </div>
...
function renderDashboardSummary(summary) {
  ...
  kpiHost.innerHTML = [
    ...
    metricItem("Zonas", kpis.zones_total, "Monitoradas e restritas"),
...
function renderDashboardInsights(summary) {
  ...
  const eventTypes = summary.event_types.slice(0, 4).map((item) => `${item.type}: ${item.count}`).join(" · ") || "sem eventos";
  document.querySelector("#dashboard-insights").innerHTML = [
    objectLine("Áreas", areas, "Distribuição das câmeras cadastradas"),
    objectLine("Tipos de evento", eventTypes, "Ranking operacional recente"),
    objectLine("Próxima ação", summary.kpis.events_open ? "Revisar eventos abertos" : "Validar zonas e regras", "Fila de trabalho sugerida"),
  ].join("");
}
  ...
  return `
    <article class="ops-row" data-event-id="${event.id}">
      <div>
        ...
        <span>${camera?.name || event.camera_id} · ${zone?.name || event.zone_id || "sem zona"} · track ${event.track_id ?? "-"}</span>
