...
class RuleRepository:
    ...
    def list(self, enabled_only: bool = False) -> list[EventRule]:
        self.ensure_defaults()
        where = "WHERE enabled = 1" if enabled_only else ""
        with connect(self.database_path) as connection:
            rows = connection.execute(
                f"SELECT * FROM visual_rules {where} ORDER BY created_at ASC"
            ).fetchall()
    ...
    def list_dicts(self) -> list[dict]:
        ...
        with connect(self.database_path) as connection:
            rows = connection.execute(
                "SELECT * FROM visual_rules ORDER BY created_at ASC"
            ).fetchall()
    ...
    def update(self, rule_id: str, updates: dict) -> dict:
        allowed = {
