    ...
    def create(
        ...
        *,
        ...
        with connect(self._db_path) as connection:
            connection.execute(
                """
                INSERT INTO zones (id, camera_id, name, type, enabled, points)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    zone_id,
            ...
            connection.commit()
        return self.get(zone_id)  # type: ignore[return-value]

    def update(self, zone_id: str, updates: dict) -> Zone | None:
