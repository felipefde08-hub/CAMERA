...
class MachineRepository:
    ...
    def list(self, camera_id: str | None = None) -> list[Machine]:
        ...
        with connect(self._db_path) as connection:
            connection.execute(
                ...
                (
                    ...
                    int(allow_idle),
                    float(min_person_distance),
                ),
            )
            connection.commit()
        return self.get(machine_id)  # type: ignore[return-value]

    def update(self, machine_id: str, updates: dict) -> Machine | None:
        allowed = {
            "name",
            "type",
            "enabled",
