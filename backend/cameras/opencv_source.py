...
class OpenCVCameraSource(CameraSource):
    def __init__(self, config: CameraConfig, capture_target: int | str) -> None:
    ...
    def connect(self) -> bool:
        ...
        capture = cv2.VideoCapture(self.capture_target)
        ...
        if not frame_result.success:
            ...
            logger.warning(
                "Camera connection opened but no valid frame was received",
                extra={"camera_id": self.config.id, "source_uri": safe_uri},
            )
            return False
    ...
    def close(self) -> None:
        ...
        if self._health.status != CameraStatus.OFFLINE:
            self._health.status = CameraStatus.OFFLINE
            logger.info("Camera stopped", extra={"camera_id": self.config.id})
...
class RTSPSource(OpenCVCameraSource):
    def __init__(self, config: CameraConfig) -> None:
        super().__init__(config, config.source_uri)
