...
def local_diagnostics() -> dict:
    ...
    with connect(settings.sqlite_path) as connection:
        ...
        ia_ativa = connection.execute(
            "SELECT COUNT(*) FROM cameras WHERE vision_enabled = 1"
        ).fetchone()[0]
        ...
        investigacoes_abertas = connection.execute(
            "SELECT COUNT(*) FROM investigations WHERE status != 'CLOSED'"
        ).fetchone()[0]
        regras_ativas = connection.execute(
            "SELECT COUNT(*) FROM visual_rules WHERE enabled = 1"
        ).fetchone()[0]
        ultima_entrega = connection.execute(
            "SELECT * FROM events ORDER BY updated_at DESC LIMIT 1"
        ).fetchone()
...
def productivity_summary(request: Request) -> dict:
    settings = _settings()
    ...
    cameras = camera_repo.list()
    mapped_machines = machine_repo.list()
    ...
    scores = [item.get("score", 100) for item in snapshots]
    return {
        "score": round(sum(scores) / len(scores), 1) if scores else 100,
        "counts": {
            "cameras": len(cameras),
            "people": total_people,
        ...
        "cameras": [
            {
                ...
                "mapped_machines": mapped_by_camera.get(camera.id, []),
                **snapshot,
            }
...
def list_evidence(limit: int = Query(100, ge=1, le=500)) -> list[dict]:
    settings = _settings()
    with connect(settings.sqlite_path) as connection:
...
def setup_demo(request: Request) -> dict:
    settings = _settings()
    camera_repo = CameraRepository(settings)
    ...
    source = "palace.mp4"
    existing = next((cam for cam in camera_repo.list() if cam.source_type == "video_file" and cam.source_uri == source), None)
    if existing is None:
    ...
    zones = zone_repo.list(camera.id)
    if not zones:
        zone_repo.create(
            camera_id=camera.id,
    ...
    saved = camera_repo.get(camera.id)
    return {
        "camera": {
            "id": saved.id,
            "name": saved.name,
...
def list_investigations(status_filter: str | None = Query(None, alias="status")) -> list[dict]:
    settings = _settings()
    where = ""
    params: list[str] = []
    if status_filter:
...
def create_investigation(payload: InvestigationCreate) -> dict:
    ...
    with connect(settings.sqlite_path) as connection:
        connection.execute(
            """
            INSERT INTO investigations (id, title, status, event_id, notes, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (
                investigation_id,
...
def update_investigation(investigation_id: str, payload: InvestigationUpdate) -> dict:
