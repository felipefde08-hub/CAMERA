...
def list_zones(
    ...
    return [zone.as_dict() for zone in repository.list(camera_id)]
...
def create_zone(
    ...
    camera = camera_repo.get(payload.camera_id)
    if camera is None:
...
def update_zone(
    ...
    existing = repository.get(zone_id)
    if existing is None:
        raise HTTPException(status_code=404, detail="Zone not found.")
    ...
    zone = repository.update(zone_id, updates)
    if zone is None:
        raise HTTPException(status_code=404, detail="Zone not found.")
    return zone.as_dict()
