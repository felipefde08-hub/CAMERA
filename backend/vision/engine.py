...
class VisionSession:
    ...
    def process(self, frame: object, frame_at: datetime) -> None:
        ...
        if not inference_required:
            ...
            with self._lock:
                ...
                if objects or not preserve_tracked_objects:
                    ...
                    self._productivity_snapshot = self._productivity.analyze(
                        self.camera_id, self._objects, timestamp
                    )
                self._events = []
            return
        ...
        productivity_snapshot = self._productivity.analyze(
            self.camera_id, objects, timestamp
        )
...
class VisionEngine:
    ...
    def start_session(self, camera_id: str) -> dict:
        ...
        with self._lock:
            ...
            self._ensure_detector_loaded_locked()
